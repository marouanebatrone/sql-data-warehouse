...
BEGIN
    ...
    TRY
        ...
        END AS sls_due_dt,
        CASE WHEN sls_sales IS NULL OR sls_sales <= 0 THEN sls_price / NULLIF(sls_quantity, 0)
            ELSE sls_sales 
        END AS sls_sales,
        sls_quantity,
        CASE WHEN sls_price IS NULL OR sls_price <= 0 THEN sls_quantity * ABS(sls_sales)
            ELSE sls_price 
        END AS sls_price
    ...
    CATCH
        ...
        PRINT '==============================='
    END CATCH
    SET @silver_e_time = GETDATE();
    PRINT 'Silver Loading Duration: ' + CAST(DATEDIFF(second, @silver_s_time, @silver_e_time) AS NVARCHAR) + ' seconds.'
    PRINT '--------------------------------------------------------------------------'
END;